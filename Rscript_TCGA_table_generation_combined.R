...
setwd("/Users/vanbaren/TCGA_rnaseq")                              #set working directory as appropriate (it must contain the source files) 
...
file_names <- list.files(path = "TCGA_rnaseq_counts", pattern = ".counts$", all.files = FALSE, full.names = TRUE)     #generate a vector listing the names of the files
...
for (sample_name in file_names) {                                                                           #start of the loop: sample files are processed 1 by 1
  ...
  buffer_name <- str_replace(sample_name, "TCGA_rnaseq_counts/", "")                                       #generate a sample name without the prefix
  buffer_name <- str_replace(buffer_name, ".counts", "")                                               #remove the suffix from this sample name
...
write.table(tibble_expr, file = "tibble_expr_counts.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)      #if necessary (to avoid repeated lengthy runs) save data as new table
...
manifest <- read.table("Downloaded_from_GDC/gdc_manifest.2018-09-06_counts.txt", header = TRUE)    #import manifest file obtained from GDC website
...
manifest <- as.tibble(manifest) %>%
  rename(file_id = id) %>%
  rename(manifest_file_id = filename)
manifest$manifest_file_id <- str_replace(manifest$manifest_file_id,".htseq.counts.gz", "")
...
tibble_expr_sample_clin <- tibble_expr_sample_clin %>%
  mutate(tibble_expr_sample_clin,patient_id = as.character(tumor_code)) %>%
...
write.table(tibble_expr_sample_clin, file = "expr_data_select_clin_annot_counts.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)