...
for (sample_name in file_names) {                                                                           #start of the loop: sample files are processed 1 by 1
  ...
  buffer_name <- str_replace(sample_name, "TCGA_rnaseq_FPKM_UQ/", "")                                       #generate a sample name without the prefix
  buffer_name <- str_replace(buffer_name, ".FPKM_UQ.txt", "")                                               #remove the suffix from this sample name
...
tibble_hgnc <- left_join(mart_annot, tibble_expr, by = "ensembl_gene_id") %>%     #join this table with the gene annotation table by matching gene identifier
  dplyr::select(2:11576) %>%                                                      #select all the columns except the gene ID column (i.e. delete that column)
...
manifest <- read.table("Downloaded_from_GDC/gdc_manifest.2018-05-23.txt", header = TRUE)    #import manifest file obtained from GDC website
...
tibble_clinannot_all <- dplyr::select(tibble_clinannot_all, -1,-3,-4,-5)                                                                #removes duplicate columns
...
write.table(tibble_expr_sample_clin, file = "expr_data_select_clin_annot_FPKM_UQ.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)